...
public class Server implements Runnable {
  ...
  public static ConcurrentHashMap<String, MySocket> userslist = new ConcurrentHashMap<>();  //Creem la llista que contindrà els usurais connectats
  MySocket mysck;
  String nickname;
  public static boolean next = false; //Estara en true quan l'usuari tingui un nom vàlid i no repetit. Llavors podrà participar en el Xat
  ...
  public static void main(String[] args) throws Exception {
    MyServerSocket mssocket = new MyServerSocket(5000);	//Mateix server host que esta declarat a client
    ...
    while(true){
      ...
      while (!next) { //next == false
        ...
        String read_nickname = mysocket.readLine();   //Es llegueix el nom d'usuari demanat per pantalla
        ...
        if (userslist.containsKey(read_nickname)) {   //Si el usuari introdueix un nom ja existent a la llista
          mysocket.println("El nom d'usuari " + read_nickname + " ja esta enregistrat.");
          ...
          new Thread(new Server(read_nickname, mysocket)).start();  //Inicialització de l'usuari al xat general
